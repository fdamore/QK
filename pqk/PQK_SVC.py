...
class PQK_OCSVC(OneClassSVM):
    ...
    def __init__(self,_fm_dict = {}, nu = 0.5, gamma = 0.5, fit_clear = True, obs = ['Z'], measure_fn = QMeasures.StateVectorEstimator,  c_kernel = 'rbf', 
                 pqk_verbose = True, *, circuit : QuantumCircuit):

        # chiama il costruttore della classe madre (OneClassSVM) e passa il kernel personalizzato
    ...
    def fit(self, X, y=None):
        ...
        super().fit(X=X)                          # qui ho cancellato y=y perche OneClassSVM non lo usa

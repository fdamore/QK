...
#used circuits
qc =  Circuits.x_encoded(n_wire=6, full_ent=False)
print(qc.draw('text'))

q_enc = QEncoding(data=X__np, obs=obs, qcircuit=qc)
en = q_enc.encode()

#q_enc.save_encoding(file_name='QC_X_OBS_M1_ENT_FALSE.csv',y_label= y_np)
q_enc.save_encoding(file_name='QC_X_OBS_M1_ENT_FALSE_2PI.csv',y_label= y_np)
